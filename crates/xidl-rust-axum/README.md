...
`xidlc gen rust-axum` generates, for each `interface`:
...
Server code is generated by default. Add client code when needed:

```bash
xidlc gen rust-axum --client ...
xidlc gen rust-axum --server --client ...
