...
async fn rest_snapshot_tests() {
    ...
    run_rest_snapshots(&vars).await;

    task.abort();
}

async fn run_rest_snapshots(vars: &serde_json::Map<String, serde_json::Value>) {
    let defs_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/rest_snapshots/defs");
    let out_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/rest_snapshots/snapshots");
...
async fn run_test(client: &reqwest::Client, test: &HttpTest, config: &SnapshotConfig) -> String {
    ...
    request_lines.push("> User-Agent: xidlc-rest-snapshot".to_string());
    ...
    req = req.header("User-Agent", "xidlc-rest-snapshot");
    for (name, value) in &test.headers {
