...
pub struct Cdr3Serializer {
    output: String,
}

pub struct Cdr3Deserializer {
    output: String,
}
...
impl SerializeVisitor for Cdr3Serializer {
    fn serialize_u8(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u8", name, "serialize");
        Ok(())
    }

    fn serialize_i8(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i8", name, "serialize");
        Ok(())
    }

    fn serialize_u16(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u16", name, "serialize");
        Ok(())
    }

    fn serialize_i16(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i16", name, "serialize");
        Ok(())
    }

    fn serialize_u32(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u32", name, "serialize");
        Ok(())
    }

    fn serialize_i32(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i32", name, "serialize");
        Ok(())
    }

    fn serialize_u64(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u64", name, "serialize");
        Ok(())
    }

    fn serialize_i64(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i64", name, "serialize");
        Ok(())
    }

    fn serialize_bool(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "bool", name, "serialize");
        Ok(())
    }

    fn serialize_f32(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "f32", name, "serialize");
        Ok(())
    }

    fn serialize_f64(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "f64", name, "serialize");
        Ok(())
    }
...
impl DeserializeVisitor for Cdr3Deserializer {
    fn deserialize_u8(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u8", name, "deserialize");
        Ok(())
    }

    fn deserialize_i8(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i8", name, "deserialize");
        Ok(())
    }

    fn deserialize_u16(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u16", name, "deserialize");
        Ok(())
    }

    fn deserialize_i16(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i16", name, "deserialize");
        Ok(())
    }

    fn deserialize_u32(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u32", name, "deserialize");
        Ok(())
    }

    fn deserialize_i32(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i32", name, "deserialize");
        Ok(())
    }

    fn deserialize_u64(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "u64", name, "deserialize");
        Ok(())
    }

    fn deserialize_i64(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "i64", name, "deserialize");
        Ok(())
    }

    fn deserialize_bool(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "bool", name, "deserialize");
        Ok(())
    }

    fn deserialize_f32(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "f32", name, "deserialize");
        Ok(())
    }

    fn deserialize_f64(&mut self, name: &str) -> Result<()> {
        emit(&mut self.output, "cdr3", "f64", name, "deserialize");
        Ok(())
    }
