...
impl<'a> crate::parser::FromTreeSitter<'a> for FixedPtType {
    fn from_node(
        ...
        assert_eq!(node.kind_id(), derive::node_id!("fixed_pt_type"));
        let mut values = Vec::new();
        for ch in node.children(&mut node.walk()) {
            if ch.kind_id() == derive::node_id!("positive_int_const") {
...
impl<'a> crate::parser::FromTreeSitter<'a> for MapType {
    fn from_node(
        ...
        assert_eq!(node.kind_id(), derive::node_id!("map_type"));
        let mut types = Vec::new();
        let mut len = None;
        for ch in node.children(&mut node.walk()) {
            match ch.kind_id() {
                derive::node_id!("type_spec") => {
                    types.push(crate::parser::FromTreeSitter::from_node(ch, ctx)?);
                }
                derive::node_id!("positive_int_const") => {
