...
- **WHEN** the OpenAPI generator renders an `@server_stream` operation with path
  or query bindings
...
The system MUST reject HTTP stream definitions that cannot be represented by the
supported OpenAPI projection.
