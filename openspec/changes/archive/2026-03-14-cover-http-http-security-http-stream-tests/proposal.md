...
- Expand `xidlc/tests` so the `http`, `http-security`, and `http-stream` specs
  each have an explicit positive and negative coverage matrix across the
  relevant generator targets.
- Add missing targeted validation tests for invalid annotation combinations,
  unsupported transport/method/shape combinations, and target-specific rejection
  paths.
- Add or extend `xidlc-examples` integration tests so generated HTTP and stream
  ...
  flows.
- Document the intended coverage boundaries in OpenSpec artifacts so future work
  can tell whether a new behavior needs fixture coverage, integration coverage,
  or both.
...
- Affected APIs: generated snapshot outputs and example integration behavior for
  HTTP, HTTP security, and HTTP stream targets.
