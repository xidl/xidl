...
- **WHEN** the OpenAPI generator renders an `@server-stream` operation with
...
- **WHEN** the OpenAPI generator encounters an `@server-stream` operation that
  is not GET or an `@client-stream` operation that is not POST
- **THEN** generation MUST fail with a validation error
