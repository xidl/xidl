...
- **WHEN** an operation is annotated with
  `@deprecated(since = "...", after = "...")` and the normalized `since`
  timestamp is later than `after`
...
- **THEN** generated Python handlers MUST bind those values according to the
  effective unary HTTP RFC rules
