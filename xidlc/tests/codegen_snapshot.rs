...
const SNAPSHOT_XIDLC_VERSION: &str = "0.0.1";
const SNAPSHOT_XIDLC_SHORTHASH: &str = "snapshot";
...
fn case_props(folder: &str, case_name: &str) -> HashMap<String, serde_json::Value> {
    ...
    HashMap::from([
        ...
        (
            String::from("xidlc_version"),
            serde_json::Value::from(SNAPSHOT_XIDLC_VERSION),
        ),
        (
            String::from("xidlc_shorthash"),
            serde_json::Value::from(SNAPSHOT_XIDLC_SHORTHASH),
        ),
    ])
}
...
#[tokio::test(flavor = "current_thread")]
async fn snapshot_props_override_generated_header_metadata() {
    ...
    let expected = format!(
        ...
        SNAPSHOT_XIDLC_VERSION, SNAPSHOT_XIDLC_SHORTHASH
    );
    assert!(
        output.contains(&expected),
        "snapshot props should override generated header metadata: {output}"
    );
}
