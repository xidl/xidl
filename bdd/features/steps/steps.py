...
def step_impl(context, lang):
    ...
    if lang == "rust" and context.protocol == "rest":
        cmd_lang = "rust-axum"
    elif lang == "rust" and context.protocol == "jsonrpc":
...
def step_impl(context, lang):
    ...
    if lang == "go":
        assert any(f.endswith(".go") for f in files)
    elif lang == "python":
...
def step_impl(context, lang):
    ...
    for f in files:
        if (
            (lang == "go" and f.endswith(".go"))
            or (lang == "python" and f.endswith(".py"))
            or (lang == "rust" and f.endswith(".rs"))
        ):
            content = open(os.path.join(context.lang_dir, f)).read()
            struct_marker = (
                "type User struct"
                if lang == "go"
                else ("class User" if lang == "python" else "struct User")
            )
            interface_marker = (
                ...
                if lang == "go"
                    ...
                    if lang == "python"
                    else "trait UserService"
                )
            )
            if struct_marker in content:
                found_struct = True
            if interface_marker in content:
                found_interface = True
    assert found_struct, f"User struct not found in {files}"
    assert found_interface, f"UserService interface not found in {files}"


@then(
    "the generated {lang} code should contain correct AddRequest struct and Calculator interface"
)
def step_impl(context, lang):
    files = os.listdir(context.lang_dir)
    found_struct = False
    ...
    for f in files:
        if f.endswith(".rs"):
            content = open(os.path.join(context.lang_dir, f)).read()
            if "struct AddRequest" in content:
                found_struct = True
            if "trait Calculator" in content:
                found_interface = True
    assert found_struct, f"AddRequest struct not found in {files}"
...
def get_module_name(lang_dir):
    ...
    for f in files:
        if f.endswith("_http.py"):
            return f[:-8]
        if f.endswith("_http.go"):
            return f[:-8]
        if f.endswith(".server.ts"):
            return f[:-10]
        if f.endswith(".rs") and not f.startswith("mod") and "jsonrpc" not in f:
            return f[:-3]
        if f.endswith(".rs") and "jsonrpc" in f:
            return f[:-3]
    return "main"
...
def copy_python_boilerplate(context, idl_name):
    ...
    assert os.path.isdir(src_dir), (
        f"missing python boilerplate for {idl_name}: {src_dir}"
    )
...
def start_python_boilerplate_server(context, idl_name, log_prefix):
    ...
    context.env["PYTHONPATH"] = os.pathsep.join(
        [os.path.abspath("python"), context.lang_dir, python_path]
    )
    context.env["PORT"] = str(context.port)
    context.server_file = os.path.join(context.lang_dir, "main.py")
    assert os.path.exists(context.server_file), (
        f"missing python server fixture: {context.server_file}"
    )
    ...
    if lang == "go":
        ...
        if os.path.exists(go_mod_path):
            ...
            content = content.replace(
                "{{GOLANG_XIDL_GO_REST_PATH}}", os.path.abspath("golang/xidl-go-rest")
            )
            content = content.replace(
                "{{GOLANG_XIDL_GO_PATH}}", os.path.abspath("golang/xidl-go")
            )
            content = content.replace(
                "{{GOLANG_XIDL_GO_CODEC_PATH}}", os.path.abspath("golang/xidl-go-codec")
            )
        ...
        shutil.copy(
            os.path.join(src_dir, "src", "main.rs"),
            os.path.join(context.lang_dir, "src", "main.rs"),
        )
        ...
        with open(cargo_toml_path, "r") as f:
            content = f.read()
        content = content.replace(
            "{{RUST_XIDL_RUST_AXUM_PATH}}", os.path.abspath("./crates/xidl-rust-axum")
        )
        content = content.replace(
            "{{RUST_XIDL_JSONRPC_PATH}}", os.path.abspath("./crates/xidl-jsonrpc")
        )
        ...
        env["CARGO_TARGET_DIR"] = os.path.join(
            os.path.abspath("."), "bdd", ".temp", "rust_target"
        )
        context.server_process = start_context_server(
            context,
            ["cargo", "run"],
...
def step_impl(context):
    ...
    for hurl_file_name in hurl_files:
        ...
        cmd = [
            ...
            "--test",
            "--variable",
            f"base_url=http://127.0.0.1:{context.port}",
            hurl_file,
        ]
        result = subprocess.run(
            cmd, capture_output=True, text=True, cwd=context.lang_dir
        )
        if result.returncode != 0:
...
def step_impl(context, name, user_id):
    resp = requests.post(
        ...
        json={"user": {"id": user_id, "name": name, "roles": ["admin"]}},
        timeout=10,
    )
...
def step_impl(context, user_id, name):
    ...
    assert resp.status_code == 200, f"Got {resp.status_code}: {resp.text}"
    data = resp.json()
    user_data = data.get("value") or data.get("return") or data
...
def step_impl(context, attr, value):
    if context.protocol == "jsonrpc":
        ...
        inner = (
            res["result"].get("value") or res["result"].get("return") or res["result"]
        )
        ...
        res = (
            data.get("value") or data.get("return") or data
            if isinstance(data, dict)
            else data
        )
        assert str(res) == value or (
            isinstance(res, dict) and str(res.get("value")) == value
        )
...
@then(
    'the client can get item {id:d} with filter "{filter_str}" and trace id "{trace_id}"'
)
...
def step_impl(context, name, age):
    ...
    assert str(age) in str(res)


@then(
    'the client gets a {status:d} error with msg containing "{expected_msg}" when requesting {method} "{path}"'
)
def step_impl(context, status, expected_msg, method, path):
    ...
    assert expected_msg.lower() in data["msg"].lower(), (
        f"Expected message containing '{expected_msg}', got '{data['msg']}'"
    )


@then(
    'the client gets a {status:d} error with msg containing "{expected_msg}" when requesting {method} "{path}" with headers'
)
def step_impl(context, status, expected_msg, method, path):
    url = f"http://127.0.0.1:{context.port}{path}"
    headers = {}
...
@then(
    'the client can upload {count:d} chunks for asset "{asset_id}" with bearer auth and get result containing "{expected}"'
)
...
@then(
    'the response header "{header}" should be "{value}" when requesting {method} "{path}" with headers'
)
def step_impl(context, header, value, method, path):
    ...
    assert resp.headers.get(header) == value, (
        f"Expected {header}: {value}, got {resp.headers.get(header)}"
    )


@then(
    'the response header "{header}" should not be present when requesting {method} "{path}" with headers'
)
def step_impl(context, header, method, path):
    ...
    assert header not in resp.headers, (
        f"Expected header {header} to be absent, but got {resp.headers.get(header)}"
    )
...
def step_impl(context, expected_body):
    assert context.last_response.text == expected_body, (
        f"Expected '{expected_body}', got '{context.last_response.text}'"
    )
...
@then(
    'the client can send flatten struct with any field payload with key "{key}" and value "{value}"'
)
def step_impl(context, key, value):
