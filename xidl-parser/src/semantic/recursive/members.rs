...
use crate::hir::{
    ConstrTypeDcl, Definition, ElementSpecTy, ModuleDcl, Specification, StructDcl, TypeDcl,
    TypeSpec, TypedefType, UnionDef,
};
...
/// Marks struct members and union cases that participate in a recursive
/// type cycle so generators can render them lazily.
...
fn collect_union_def_edges(
    ...
    for case in &def.case {
        if let ElementSpecTy::TypeSpec(ty) = &case.element.ty {
            if let Some(target) = direct_struct_target(module_path, ty, names) {
                edges.push((owner.clone(), join_path(&target)));
            }
        }
    }
