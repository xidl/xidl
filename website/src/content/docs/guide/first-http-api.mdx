...
```bash
xidlc gen --out-dir server rust-axum hello_world.idl
```
...
```bash
xidlc gen --out-dir client rust-axum --client hello_world.idl
```
...
A runnable version of this tutorial lives in the repository under
`xidlc-examples/` — see `api/rest/hello_world.idl`,
`examples/hello_world_server.rs`, and `examples/hello_world_client.rs`:

```bash
cargo run -p xidlc-examples --example hello_world_server
# in another terminal
cargo run -p xidlc-examples --example hello_world_client
```
