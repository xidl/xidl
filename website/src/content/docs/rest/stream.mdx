...
- `@upgrade(protocol = "websocket", ...)`: explicit RFC 6455 WebSocket with
  optional `codec` / `subprotocol` / `heartbeat` / `max_message` (see the
  [HTTP Upgrade / WebSocket RFC](../../rfc/http-upgrade/)).
