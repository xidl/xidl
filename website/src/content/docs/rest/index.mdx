...
XIDL's RESTful code generation covers three parts:
...
For the TypeScript server contract (including Next.js App Router integration),
see [TypeScript REST Server](./typescript-server/).
...
Use the `@request` and `@response` annotations to specify the request and response media types. Instead of a full MIME type, they take a format token: `json`, `urlencoded`, or `msgpack`. Both support interface-level defaults and method-level overrides.
