...
## Relationship to WebSocket Upgrade

`@bidi_stream` is the abstract bidirectional stream. On HTTP it maps to a
WebSocket session with the default profile defined by the
[HTTP Upgrade / WebSocket RFC](../http-upgrade/).

For explicit RFC 6455 options (`codec`, `subprotocol`, `heartbeat`,
`max_message`), use `@upgrade(protocol = "websocket", ...)` instead.
