...
`xidl-rust-axum` is the runtime crate for code generated by
`xidlc --lang rust-axum`. It provides a minimal server/client foundation on top
of `axum` + `reqwest`.

- Runtime types: `Server` / `Service` / `Client` / `Request` / `Error`
- Codegen: `xidlc` generates the interface trait, server wrapper, and client
  wrapper
- Re-exports: `axum` / `reqwest` / `serde` / `serde_json` (so generated code can
...
The repo includes this example in `xidlc-examples` (which also contains jsonrpc
examples):
